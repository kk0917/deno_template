...
for await (const request of server){
    const authedReq = confirmCorsAuth(request);
    const [requestType, contentType] = getParams(authedReq);
    const errorMessage = isBadRequest(contentType, requestType);
    console.log('good request!');
}
function confirmCorsAuth(request1) {
    request1.headers.append('Access-Control-Allow-Origin', '*');
    if (request1.method === "OPTIONS") {
        request1.respond({
            status: 200,
            headers: new Headers({
...
function getParams(request1) {
    const searchParams = new URLSearchParams(request1.url), requestType = searchParams.get('type'), contentType = request1.headers.get('Content-Type');
    return [
        requestType,
        contentType
    ];
}
function isBadRequest(contentType, type) {
    let message1 = "";
    if (contentType !== "application/json") {
        message1 = `Bad Request: Unknown content type: ${contentType}`;
    }
    if (!type) {
        message1 = "Bad Request: JSON is invalid, or missing type param..";
    }
    return message1;
}
